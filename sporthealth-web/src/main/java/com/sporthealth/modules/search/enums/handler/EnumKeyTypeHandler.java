package com.sporthealth.modules.search.enums.handler;

import com.sporthealth.modules.search.enums.IEnum;
...
public class EnumKeyTypeHandler extends BaseTypeHandler<IEnum> {
     ...
     * @return code对应的枚举类
     */
    private IEnum locateEnum(int code) {
        for(IEnum status : enums) {
            if(status.getKey() == Integer.valueOf(code)) {
                return status;
            }
        }
